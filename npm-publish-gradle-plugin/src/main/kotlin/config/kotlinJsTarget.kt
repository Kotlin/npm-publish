...
@Suppress("LongMethod")
internal fun Project.configure(target: KotlinJsTargetDsl): Unit =
    ...
    if (target !is KotlinJsIrTarget) {
      ...
      extensions.getByType(NpmPublishExtension::class.java).packages.register(target.name) { pkg ->
          ...
          provider<JsIrBinary> {
            when (val it = target.binaries.find { it.mode == KotlinJsBinaryMode.PRODUCTION }) {
              ...
              is LibraryWasm -> {
                warn {
                  """
                  Kotlin/JS LibraryWasm binaries are not supported npm package targets.
        ...
        val processResourcesTask =
          mainCompilation.flatMap { tasks.named<Copy>(it.processResourcesTaskName) }

        val outputFile = binary.flatMap(JsIrBinary::mainFile)
        val typesFile =
          outputFile.map { it.asFile.parentFile.resolve("${it.asFile.nameWithoutExtension}.d.ts") }
        ...
        pkg.types.convention(
          sysProjectEnvPropertyConvention(
            pkg.prefix + "types",
            typesFile
              .map<String> { it.takeIf(File::exists)?.name.unsafeCast() }
      ...
      info {
        "Automatically registered [${target.name}] NpmPackage for [${target.name}] Kotlin/JS target"
      }
    }
  }

private fun KotlinJsCompilation.relatedConfigurationNames() =
  listOfNotNull(
